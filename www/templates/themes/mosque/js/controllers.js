...
appControllers.controller('mosquelistCtrl', function ($scope, $http, $state, $stateParams,$ionicLoading, $ionicHistory,WORDPRESS_API_URL,positionService,mosque_service) {
     ...
     if (data.length == 0) {
            ...
            // If have data it will store feed data to  $scope.feedList variable to show in feed.
                ...
                for (var postItem = 0; postItem < data.length; postItem++) {
                    $scope.mosquelist.push(data[postItem]);
			         console.log($scope.mosquelist);
                }
...
appControllers.controller('mosquedetailCtrl', function ($scope, $http,  $stateParams,$state, $ionicLoading, $timeout, $ionicModal, $ionicPopup, $mdToast, Postlist_service,WORDPRESS_API_URL) {
    ...
    var dataURL = WORDPRESS_API_URL + "wp/v2/" + $stateParams.wordpressUrl + "/" + $stateParams.postDetail;
		$http.get(dataURL, {
        params: {}
            ...
            var MediaURL = WORDPRESS_API_URL + "wp/v2/media/" + $scope.restaurant.featured_media;
            $http.get(MediaURL, {
                params: {}
            }).then(function(result) {
                    //success retrieve data by calling http service. it will store comment data to $scope.comments.
            ...
            /////comments////////
            var MediaURL = WORDPRESS_API_URL + "wp/v2/comments?post=" + $scope.restaurant.id;
            $http.get(MediaURL, {
                params: {}
            }).then(function(result) {
                    //success retrieve data by calling http service. it will store comment data to $scope.comments.
...
// Controller of WordPress Post Page.
appControllers.controller('detailCtrl', function ($scope, $http, $timeout, $stateParams) {
    // This function is the first activity in the controller.
    // It will initial all variable data and let the function works when page load.
    $scope.initialForm = function () {
